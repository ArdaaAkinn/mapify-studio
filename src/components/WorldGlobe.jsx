...
// These maps are individual country maps hidden in world view, revealed in europe zoom
const HIDDEN_IN_WORLD = new Set(["germany", "france", "italy", "spain", "greece", "turkey", "russia", "uk"]);

// Map GeoJSON country names (from europe.json) to map catalog IDs
...
// Green used for generic European countries (no specific map)
...
const simplifyRing = (ring, maxPoints = 60) => {
...
export default function WorldGlobe({ maps, onSelectMap, onSelectGroup, activeMapId, globeGroup }) {
  ...
  // Shared projection state — written by render loop, read by hit-test
  ...
  // Trigger zoom animation when globeGroup changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Cache — never changes
    const oceanGradient = context.createRadialGradient(CENTER - 120, CENTER - 150, 20, CENTER, CENTER, 326);
    oceanGradient.addColorStop(0, "#eff6ff");
    oceanGradient.addColorStop(0.58, "#60a5fa");
    oceanGradient.addColorStop(1, "#1d4ed8");

    // Pre-build lookup for europe zoom mode
    ...
    // Pre-clip Russia's simplified feature to European part only.
    // The main polygon spans 27°E–180°E; we cut coordinates east of 80°E and
    // drop far-eastern island polygons entirely. Runs once, not per frame.
    const europeFeatures = europeRegion?.simplifiedGeoData.features.map(feat => {
      ...
      const clipped = feat.geometry.coordinates
        .map(polygon => {
          ...
          if (minL > 80 || maxL < -25) return null;   // pure far-east polygon, skip

          if (maxL > 80) {
            // Clip the ring at 80°E
            const cut = ring.filter(([lon]) => lon < 80);
            if (cut.length < 3) return null;
            const f = cut[0], l = cut[cut.length - 1];
            if (f[0] !== l[0] || f[1] !== l[1]) cut.push(cut[0]); // close ring
            return [cut];
          }
          return polygon;
        })
    ...
    const render = (now) => {
      ...
      // — WORLD mode (and during any animation) —
      ...
      // — EUROPE ZOOM mode (only after animation settles) —
      } else {
        // Draw all europe.json features individually with color-coding.
        ...
        // Labels for specific country maps only
        const rotation = projection.rotate();
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event) => {
      ...
      // In europe mode, only specific maps get a hover highlight (not the generic europe continent)
    ...
    const handleClick = (event) => {
      ...
      if (!currentGroup) {
        ...
        // Europe zoom: open that map's studio
        onSelectMapRef.current(map.id);
      }
    };
  ...
  return (
    <canvas
      className={`world-globe${globeGroup === "europe" ? " world-globe--zoomed" : ""}`}
      ref={canvasRef}
